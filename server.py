...
def send_end_game():
    ...
    for c in COLORS:
        result[c] = list(BALL_STATE.values()).count(c)
...
def handle_client(conn, addr, player_id, color):
    global game_over
...
def main():
    global player_id_counter, game_started
    ...
    while True:
        ...
        with lock:
            ...
            threading.Thread(target=handle_client, args=(conn, addr, player_id, color), daemon=True).start()
            if not game_started:
                game_started = True
                for c in CLIENTS:
                    try:
                        c.sendall(json.dumps({"type": "start"}).encode())
                    except:
                        pass
